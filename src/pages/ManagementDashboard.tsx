...
export default function ManagementDashboard() {
 ...
 const subtitle=(v:DashboardView)=>v==='cohort'?text('同一注册批次的六项转化','Six milestones for one registration cohort'):v==='followup'?text('外呼、跟进与原因','Calls, follow-up and reasons'):text('支付人数、收入与 AOV','Payers, revenue and AOV')
 ...
 <p>{text('销售跟进：当前状态不受活动日期影响；本期活动按通话、预约创建、结果登记或支付日期统计。预约出席沿用销售预约登记。预约计划日按计划到场日期另查，未登记不会自动算作未出席。','Follow-up: current states ignore activity dates. Activity uses call, booking creation, result recording or payment dates. Attendance refers to sales appointments. The schedule uses planned dates; unrecorded attendance is not a no show.')}</p>
 <p>{text('关闭原因：电话拒绝、咨询后关闭分别统计，历史未知单列。待支付顾虑不改变为关闭。','Closures: phone-stage rejection and post-consultation closure are separate; unknown historical stages stay separate. Payment concerns do not close a lead.')}</p>
