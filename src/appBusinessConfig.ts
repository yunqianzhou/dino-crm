...
export function planErrors(p:VisualPlan):string[]{return planSlots(p).flatMap(s=>[...validateContent(s,effectiveContent(p,s.id)),...translationErrors(s,effectiveContent(p,s.id))].map(e=>`${s.page} / ${s.name}：${e}`))}
