import { TRIAL_LESSON_SLOT, trialLessonErrors } from './appTrialLessonConfig'
...
export function planSlots(p:VisualPlan,scopes?:string[]){const flow=effectiveContent(p,p.flowId).flow;const refs=[...new Set((flow?.nodes??[]).filter(n=>n.page==='paywall').map(n=>n.configRef).filter(Boolean))] as string[];const slots=registry.filter(s=>!s.flow||s.id===p.flowId).flatMap(s=>s.node.startsWith('paywall')?[s,...refs.map(ref=>getSlot(`${s.id}@${ref}`))]:[s]);return scopes?slots.filter(s=>scopes.includes(scopeOf(s))):slots}
export function pageSlots(p:VisualPlan,node:string,ref?:string){return registry.filter(s=>s.node===node&&!s.flow).map(s=>ref?getSlot(`${s.id}@${ref}`):s)}

export function fullPlan(p:VisualPlan):Plan {return planSlots(p).reduce((base,s)=>contentPlan(effectiveContent(p,s.id),s,base),newPlan())}
export function effectiveContent(p:VisualPlan,id:string):UnitContent {const s=getSlot(id),c=p.contents[id]??p.contents[id.split('@')[0]]??initialContent(s);return s.flow?{...c,flow:c.flow??makeFlow(c.steps??[],flowStandard(id))}:c}
export function planMinimum(p:VisualPlan){return Object.values(p.minimums??{}).sort(compareVersions).slice(-1)[0]}
export function planErrors(p:VisualPlan,target?:Target):string[]{return planSlots(p).flatMap(s=>[...validateContent(s,effectiveContent(p,s.id)),...translationErrors(s,effectiveContent(p,s.id)),...(s.id===TRIAL_LESSON_SLOT?trialLessonErrors(effectiveContent(p,s.id).details.settings.lessonVersionId,target):[])].map(e=>`${s.page} / ${s.name}：${e}`))}
...
function addVersions(store:BusinessStore,p:VisualPlan,minVersion:string,name:string,kind:Definition['kind'],actor:string,previous:string[]=[],scopes?:string[]){let next=store;const bindings=[];for(const s of planSlots(p,scopes)){const prior=previous.map(id=>next.versions.find(v=>v.id===id)).find(v=>v&&next.definitions.find(d=>d.id===v.definitionId)?.slotId===s.id);const old=prior&&next.definitions.find(d=>d.id===prior.definitionId);const content=effectiveContent(p,s.id);if(p.minimums?.[s.id]&&compareVersions(minVersion,p.minimums[s.id])<0)throw Error(`${s.page} / ${s.name} 的最低 App 版本不能低于 ${p.minimums[s.id]}。`);if(prior&&prior.minVersion===minVersion&&JSON.stringify(prior.content)===JSON.stringify(content)){bindings.push({definitionId:prior.definitionId,versionId:prior.id});continue}const d:Definition=old??{id:makeId(),slotId:s.id,name:`${name} · ${s.page} · ${s.name}`,description:'由可视化方案自动维护',kind,status:'ENABLED',createdAt:new Date().toISOString()};next=saveVersion(next,d,content,minVersion,actor,'可视化方案保存') as BusinessStore;bindings.push({definitionId:d.id,versionId:latestVersion(next,d.id).id})}return {store:next,bindings}}
export function publishOnline(store:BusinessStore,p:OnlinePlan,actor:string):BusinessStore {
 p={...p,plan:withTrialLessonVersion(p.plan)}
 const errors=[...infoErrors(p),...planErrors(p.plan,audienceTarget(p.audience,p.minVersion)),...(validVersion(p.minVersion)&&planMinimum(p.plan)&&compareVersions(p.minVersion,planMinimum(p.plan)!)<0?[`最低 App 版本不能低于原配置要求的 ${planMinimum(p.plan)}。`]:[])];if(errors.length)throw Error(errors.join('\n'))
...
export function copyForRelaunch(e:AppExperiment,v:ExperimentPlans){const fresh=newExperiment();const plans:ExperimentPlans['plans']={};fresh.value={...clone(e),id:makeId(),sourceExperimentId:e.id,name:`${e.name}（重新发起）`,status:'DRAFT',salt:makeId(),startedAt:'',endedAt:'',scheduledStartAt:new Date().toISOString(),scheduledEndAt:new Date(Date.now()+7*86400000).toISOString(),groups:e.groups.map(g=>{const id=makeId();plans[id]=clone(v.plans[g.id]);return {...g,id,bindings:[]}})};fresh.visual={...clone(v),plans};return fresh}
// Only the tested scopes overlay the current online plan. Other pages keep following online changes.
export function mergeTestedPlan(online:VisualPlan,group:VisualPlan,scopes:string[]):VisualPlan{
 const result=clone(online);if(scopes.includes('flow'))result.flowId=group.flowId;
 for(const slot of planSlots(group,scopes)){result.contents[slot.id]=clone(effectiveContent(group,slot.id));if(group.minimums?.[slot.id])result.minimums={...result.minimums,[slot.id]:group.minimums[slot.id]};}
 return result
}

function withTrialLessonVersion(plan:VisualPlan):VisualPlan{const p=clone(plan);p.contents[TRIAL_LESSON_SLOT]=clone(effectiveContent(p,TRIAL_LESSON_SLOT));return p}
