...
export function validateFlow(g:FlowGraph):string[]{
 const errors:string[]=[],nodes=g.nodes
 ...
 for(const [i,n] of nodes.entries())if(n.page==='paywall')for(const [key,label] of [['successTo','支付成功'],['failureTo','支付失败／取消']] as const){const target=nodes.findIndex(x=>x.id===n[key]);if(target<0)errors.push(`第 ${i+1} 步购买页：请选择${label}后的去向。`);else if(target<=i)errors.push(`第 ${i+1} 步购买页：${label}分支须指向后续节点，不能循环或重入。`)}
