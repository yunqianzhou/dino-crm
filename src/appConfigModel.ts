...
import { devicePlatforms, platformOverlap, clone, makeId, newPlan, templates, validVersion, versionMatches } from './abTestConfig'
import type { Plan, Platform, Target } from './abTestConfig'
import { defaultPageDetails, pageDetails, validAssetSource } from './abPageConfig'
import type { PageDetails } from './abPageConfig'

export const APP_CONFIG_KEY = 'dinoai_app_config_workbench_v3'
...
export const registry: Slot[] = [
 ...
 slot('auth','注册登录','app.login.methods','主次登录方式','login',{settings:['mainMethods','subMethods'],pending:'登录方式仍为协议示例；正式端与版本能力表、按钮动效字段待 APP 提供。'}),
 ...
 ...(['name','age','level','goal'] as const).flatMap(node => {
  ...
  return [slot(node,page,`${key}.question_title`,'问题标题','copy',{copy:['title']}),slot(node,page,`${key}.question_body`,'问题说明','copy',{copy:['body']}),
   ...(node==='name' ? [slot(node,page,`${key}.input_placeholder`,'输入提示','name',{texts:['placeholder']}),slot(node,page,`${key}.recommended_names`,'推荐名称','name',{lists:['names']}),slot(node,page,`${key}.dino_guide`,'Dino 引导与语音','guide',{texts:['guide'],assets:['guide','guideAudio'],settings:['showGuide']})] : [slot(node,page,`${key}.options`,'选项文案与内容','options',{lists:['options'],pending: node==='goal'?'仅选择既有演示目标；正式选项值由 APP 字典提供，不支持随意生成业务编号。':'编号、数量、顺序与默认状态由 APP 业务字典决定；当前为示例。'})]),button(node,page,`${key}.continue_button`),slot(node,page,`${key}.background`,'页面背景','bg',{assets:['background']})]
 }),
 slot('paywall','主购买页','app.paywall_main.slideshow','页面介绍轮播','slides',{lists:['slides']}),
 slot('paywall','主购买页','app.paywall_main.member_plan_gourp','会员档位、权益与周期','members',{members:true,pending:'按 APP 示例配置 Pro / Max 及月度 / 年度。SKU、默认套餐和字段拼写待接口统一；这里不手填商品价格。'}),
 slot('paywall','主购买页','app.paywall_main.purchase_button','购买按钮','button',{copy:['button','animation']}),
 slot('paywall','主购买页','app.paywall.main.limited_offer','活动横条','offer',{texts:['offerTitle'],assets:['offer'],settings:['offerMode'],pending:'活动配置键命名待 APP 统一；倒计时期限由现有优惠业务决定。'}),
 ...(['retention-promo','retention-regular'] as const).flatMap(node => [
  ...
  slot(node,node==='retention-promo'?'挽留支付 · Promo':'挽留支付 · 普通',`${node}.products`,'商品配置','products',{settings:['period','productMode','skus'],pending:'商品字段尚未定稿，当前仅演示周期或已上线商品选择。'}),
...
export type UnitContent = { copy: Record<string, string>; details: PageDetails; translations: Plan['translations']; pageTranslations: Plan['pageTranslations']; steps?: string[]; flow?:FlowGraph }
...
export type Audience = { countries: string[]; platforms: Platform[]; cities: string[]; channels: string[] }
...
export type AppExperiment = { id:string; name:string; audience:Audience; minVersion:string; status:ExperimentStatus; groups:Group[]; startedAt:string; endedAt:string; salt:string; legacyTraffic?:number }
...
export const audience = ():Audience => ({countries:['SA'],platforms:['android_app'],cities:[],channels:[]})
export const audienceTarget = (a:Audience,minVersion='1.8.0'):Target => ({countries:a.countries,platforms:a.platforms,versions:[{operator:'>=',value:minVersion}]})
export const getSlot = (id:string) => registry.find(s=>s.id===id)!
export function appPlan(node:string):Plan {
 const plan=newPlan(); const d=pageDetails(plan,node)
 ...
 plan.pages={...plan.pages,[node]:d};return plan
}
export function extractContent(plan:Plan,s:Slot):UnitContent {
 ...
 return {copy:pick(plan.copy[s.node]??{},s.copy),details,translations:Object.fromEntries(Object.entries(plan.translations).map(([lang,pages])=>[lang,{[s.node]:pick(pages[s.node]??{},s.copy)}])) as Plan['translations'],pageTranslations:Object.fromEntries(Object.entries(plan.pageTranslations??{}).map(([lang,pages])=>[lang,{[s.node]:Object.fromEntries(Object.entries(pages[s.node]??{}).filter(([k])=>relevant(k)))}]))}
}
export function contentPlan(content:UnitContent,s:Slot,base=appPlan(s.node)):Plan {
 const plan=clone(base);const d=pageDetails(plan,s.node)
 plan.copy[s.node]={...plan.copy[s.node],...content.copy}
 plan.pages={...plan.pages,[s.node]:{texts:{...d.texts,...content.details.texts},assets:{...d.assets,...content.details.assets},lists:{...d.lists,...content.details.lists},settings:{...d.settings,...content.details.settings},...(content.details.members?{members:content.details.members}:{})}}
...
export function initialContent(s:Slot):UnitContent {
 ...
 if(s.flow&&!s.id.endsWith('double_paywall'))c.flow=makeFlow(c.steps??[],flowStandard(s.id))
 return c
}
export type Member = { id:number; mode:'table'|'asset'; asset: import('./abPageConfig').Asset; columns:{id:string;label:string}[]; rows:{id:string;label:string;values:Record<string,boolean>}[]; cycles:{id:'MONTH'|'YEAR';label:string;priceSize:number;priceColor:string;original:boolean}[] }
export function defaultMembers():Member[] {return [1,2].map(id=>({id,mode:'table',asset:{mode:'inherit'},columns:[{id:'pro',label:'Pro'},{id:'max',label:'Max'}],rows:[{id:'ai_lesson',label:'AI 课程',values:{pro:true,max:true}},{id:'speaking_practice',label:'口语练习',values:{pro:false,max:true}}],cycles:[{id:'MONTH',label:'月度',priceSize:16,priceColor:'#27344a',original:false},{id:'YEAR',label:'年度',priceSize:16,priceColor:'#27344a',original:true}]}))}
...
export function validateContent(s:Slot,c:UnitContent):string[] {
 ...
 for(const [key,text] of Object.entries({...c.copy,...d.texts}))if(key!=='animation'&&/\{[^}]+\}/.test(text)&&/\{(?!nickname\})[^}]+\}/.test(text))errors.push('当前 APP 协议仅明确 {nickname} 变量，请移除未定义变量。')
 if(s.lists?.includes('slides')){if(!d.lists.slides?.length)errors.push('图文轮播至少保留一组。');for(const [i,item] of (d.lists.slides??[]).entries()){required(item.label,`第 ${i+1} 组标题`);required(item.body,`第 ${i+1} 组正文`)}}
 if(s.copy)for(const key of s.copy)if(key==='animation'){if(!['static','breathe'].includes(c.copy[key]))errors.push('APP 当前协议支持静态或呼吸动效。')}else required(c.copy[key],key==='title'?'标题':key==='body'?'说明文案':'按钮文案')
 for(const key of s.texts??[])if(!(key==='guide'&&!d.settings.showGuide)&&!(key==='offerTitle'&&d.settings.offerMode!=='countdown'))required(d.texts[key],({accountHint:'已有账号引导',accountAction:'登录入口',placeholder:'输入提示',guide:'引导文案',badge:'宣传标签',offerTitle:'倒计时标题'} as Record<string,string>)[key]??key)
 if(s.id==='app.login.methods'){const all=[...(d.settings.mainMethods as string[]??[]),...(d.settings.subMethods as string[]??[])];if(!all.length)errors.push('至少选择一种登录方式。');if(new Set(all).size!==all.length)errors.push('主次登录方式不能重复。')}
 if(d.settings.accountAnimation&&!['static','breathe'].includes(String(d.settings.accountAnimation)))errors.push('登录入口仅支持静态或呼吸动效。')
 ...
 if(s.flow){const steps=c.steps??[];if(!steps.length)errors.push('流程步骤不能为空。');if(steps[0] !== (s.id.endsWith('register_first')?'auth':'value'))errors.push('流程起点必须与所选注册流程一致。');if(steps[steps.length-1]!=='home')errors.push('流程终点必须是首页。');if(steps.slice(0,-1).includes('home'))errors.push('首页只能作为最后一步。');if(!steps.includes('auth'))errors.push('首次使用流程须保留注册登录步骤。');if(steps.some(x=>!['value','auth','name','age','level','goal','teacher','lesson','report','plan','paywall','home'].includes(x)))errors.push('流程包含未注册的页面。')}
 return Array.from(new Set(errors))
}
export function validateAudience(a:Audience):string[]{return [...(!a.countries.length?['请选择国家。']:[]),...(!a.platforms.length?['请选择终端。']:[]),...(a.platforms.some(p=>!devicePlatforms.some(d=>d.value===p))?['请选择具体手机或 Pad 终端，旧配置需要重新确认设备范围。']:[])]}
...
export function audienceOverlap(a:Audience,b:Audience){return intersection(a.countries,b.countries,true)&&platformOverlap(a.platforms,b.platforms)&&intersection(a.cities,b.cities)&&intersection(a.channels,b.channels)}
export const releaseState=(r:Release,now=Date.now())=>r.status==='ACTIVE'&&r.endAt&&Date.parse(r.endAt)<=now?'ENDED':r.status
export function validateRelease(store:AppConfigStore,r:Release):string[]{const d=store.definitions.find(x=>x.id===r.definitionId);const errors=[...validateAudience(r.audience)];if(!r.name.trim())errors.push('请填写投放名称。');if(!d||d.kind!=='DIRECT'||d.status!=='ENABLED'||!latestVersion(store,d.id))errors.push('请选择有可用版本的启用普通配置。');if(!Number.isFinite(r.priority))errors.push('投放优先级须为数字。');if(r.startAt&&!Number.isFinite(Date.parse(r.startAt))||r.endAt&&!Number.isFinite(Date.parse(r.endAt)))errors.push('投放时间无效。');if(r.startAt&&r.endAt&&Date.parse(r.startAt)>=Date.parse(r.endAt))errors.push('结束时间必须晚于开始时间。');if(r.endAt&&Date.parse(r.endAt)<=Date.now())errors.push('结束时间必须晚于当前时间。');if(d&&store.releases.some(other=>other.id!==r.id&&releaseState(other)==='ACTIVE'&&store.definitions.find(x=>x.id===other.definitionId)?.slotId===d.slotId&&audienceOverlap(r.audience,other.audience)&&Math.max(r.startAt?Date.parse(r.startAt):0,other.startAt?Date.parse(other.startAt):0)<Math.min(r.endAt?Date.parse(r.endAt):Infinity,other.endAt?Date.parse(other.endAt):Infinity)))errors.push('同一配置项存在范围与时间重叠的有效投放，请先结束旧投放或调整范围。');return errors}
export function validateExperiment(store:AppConfigStore,e:AppExperiment):string[]{const errors=[...validateAudience(e.audience)];if(!e.name.trim())errors.push('请填写实验名称。');if(!validVersion(e.minVersion))errors.push('请填写有效最低 App 版本，如 1.8.0。');if(e.groups.length<2)errors.push('至少设置两个分组。');if(e.groups.filter(g=>g.control).length!==1)errors.push('必须且只能有一个对照组。');if(e.groups.reduce((n,g)=>n+g.weight,0)!==10000||e.groups.some(g=>!Number.isInteger(g.weight)||g.weight<=0))errors.push('组内比例必须大于 0%，合计 100%。');if(new Set(e.groups.map(g=>g.name.trim())).size!==e.groups.length||e.groups.some(g=>!g.name.trim()))errors.push('分组名称不能为空或重复。');if(!e.groups.some(g=>g.bindings.length))errors.push('至少为一个实验组绑定配置版本。');for(const g of e.groups){const slots:string[]=[];for(const b of g.bindings){const d=store.definitions.find(x=>x.id===b.definitionId),v=store.versions.find(x=>x.id===b.versionId&&x.definitionId===b.definitionId);if(!d||d.kind!=='EXPERIMENT'||!v||d.status!=='ENABLED'){errors.push(`${g.name}：绑定版本不可用。`);continue}slots.push(d.slotId);if(validVersion(e.minVersion)&&compareVersions(e.minVersion,v.minVersion)<0)errors.push(`${g.name}：最低 App 版本不能低于「${d.name}」V${v.no} 的 ${v.minVersion}。`);errors.push(...validateContent(getSlot(d.slotId),v.content).map(x=>`${g.name} / ${d.name}：${x}`))}if(new Set(slots).size!==slots.length)errors.push(`${g.name}：同一配置项不能重复绑定。`)}const boundSlots=e.groups.flatMap(g=>g.bindings.map(b=>store.definitions.find(d=>d.id===b.definitionId)?.slotId));if(store.experiments.some(x=>x.id!==e.id&&['RUNNING','PAUSED'].includes(x.status)&&audienceOverlap(x.audience,e.audience)&&x.groups.some(g=>g.bindings.some(b=>boundSlots.includes(store.definitions.find(d=>d.id===b.definitionId)?.slotId)))))errors.push('有范围重叠的实验已绑定相同配置项，请调整范围或先结束该实验。');return [...new Set(errors)]}
...
export const statusLabels:Record<string,string>={DRAFT:'草稿',READY:'就绪',RUNNING:'进行中',PAUSED:'已暂停',ENDED:'已结束',TERMINATED:'已终止',ACTIVE:'已生效',ENABLED:'启用',DISABLED:'停用'}
...
export function seedAppStore():AppConfigStore{
 ...
 for(const slotId of ['app.login.slideshow','app.login.methods','app.paywall_main.member_plan_gourp','app.flow.register_first']){const s=getSlot(slotId);const def:Definition={id:makeId(),slotId,name:`${s.page} · ${s.name}（示例）`,description:'APP 协议示例，可复制创建实验配置。',kind:'DIRECT',status:'ENABLED',createdAt:new Date().toISOString()};store=saveVersion(store,def,initialContent(s),'1.8.0','示例数据','初始示例');store.releases.push({id:makeId(),name:`${s.page}默认投放（示例）`,definitionId:def.id,audience:audience(),priority:0,startAt:'',endAt:'',status:'ACTIVE'})}
 return store
}
...
export function serializeContent(s:Slot,c:UnitContent,namespace:string) {
 ...
 else if(s.section==='button')config={i18n_key:i18n('button',c.copy.button),animation:c.copy.animation==='breathe'?'BREATH':'STATIC'}
 else if(s.section==='bg')config={asset_url:asset(d.assets.background)}
 else if(s.section==='account')config={hint_i18n_key:i18n('accountHint',d.texts.accountHint),action_i18n_key:i18n('accountAction',d.texts.accountAction),animation:d.settings.accountAnimation==='breathe'?'BREATH':'STATIC'}
 else if(s.section==='login'){const methods:Record<string,string>={google:'GOOGLE',apple:'APPLE',phone:'MOBILE',facebook:'FACEBOOK',kakao:'KAKAO'};config={main_methods:(d.settings.mainMethods as string[]).map(x=>methods[x]),sub_methods:(d.settings.subMethods as string[]).map(x=>methods[x])}}
 ...
 else if(s.lists?.includes('options'))config={options:d.lists.options.map(x=>({option_value:x.id,option_i18n_key:i18n(`options.${x.id}.label`,x.label),...(s.node==='goal'?{icon_url:asset(x.asset)}:{})}))}
 else if(s.section==='offer')config={type:d.settings.offerMode==='asset'?'ASSET':'COUNTDOWN',...(s.node.startsWith('retention')?{countdown_title_i18n_key:d.settings.offerMode==='countdown'?i18n('offerTitle',d.texts.offerTitle):''}:{title_i18n_key:d.settings.offerMode==='countdown'?i18n('offerTitle',d.texts.offerTitle):''}),asset_url:d.settings.offerMode==='asset'?asset(d.assets.offer):''}
 else if(s.section==='products')config={prototype_pending_product_protocol:clone(d.settings)}
