import { TRIAL_LESSON_SLOT, defaultTrialLessonVersionId, trialLessonErrors } from './appTrialLessonConfig'
...
export const registry: Slot[] = [
 slot('lesson','体验课',TRIAL_LESSON_SLOT,'课程版本','lesson',{settings:['lessonVersionId']}),
...
export const testPageOptions=[{value:'flow',label:'首次使用流程'},{value:'value',label:'首启价值页'},{value:'auth',label:'前置注册登录'},{value:'auth-after',label:'后置注册登录'},{value:'name',label:'孩子称呼'},{value:'age',label:'孩子年龄'},{value:'level',label:'英语水平'},{value:'goal',label:'学习目标'},{value:'lesson',label:'体验课版本'},{value:'paywall',label:'主购买页'},{value:'retention-promo',label:'Promo 挽留'},{value:'retention-regular',label:'普通挽留'}];
...
export function appPlan(node:string):Plan {
 ...
 if(node==='lesson')d.settings.lessonVersionId=defaultTrialLessonVersionId;
...
export function validateContent(s:Slot,c:UnitContent):string[] {
 ...
 if(s.section==='lesson')return trialLessonErrors(d.settings.lessonVersionId)
...
export function serializeContent(s:Slot,c:UnitContent,namespace:string) {
 ...
 if(s.section==='lesson')config={lesson_version_id:d.settings.lessonVersionId,selection_policy:'FIXED_VERSION',in_progress_policy:'KEEP_EXISTING_COURSE'}
 else if(s.flow&&c.flow)config=serializeFlow(c.flow)
