...
export default function DashboardPayments({ population, orders, filters, rangeLabel }: Props) {
  ...
  const grouping = query.get('revenueGroup') === 'date' ? 'date' : 'cc'
  ...
  const rows = [...new Set(matchingOrders.map(groupKey))].sort((a, b) => grouping === 'date' ? b.localeCompare(a) : a.localeCompare(b)).map(id => ({
    id, name: groupName(id), ...dashboardPaymentSummary(matchingOrders.filter(order => groupKey(order) === id))[0],
  }))
  const clear = (next: URLSearchParams) => ['paymentDetail', 'paymentValue', 'paymentView', 'paymentUser'].forEach(key => next.delete(key))
  ...
  const open = (view: 'users' | 'orders', id?: string) => {
    const next = new URLSearchParams(query)
    clear(next)
  ...
  const close = () => { const next = new URLSearchParams(query); clear(next); setQuery(next) }
  ...
  const detailOrders = matchingOrders.filter(order => !selectedGroup || groupKey(order) === selectedGroup)
  ...
  return <Card title={d('revenueTitle')} className="dashboard-payments" extra={<span className="dashboard-section-note">{d('paidDate')} · {rangeLabel} · UTC+7</span>}>
    ...
    <Table rowKey="id" size="middle" dataSource={rows} scroll={{ x: 850 }} pagination={rows.length > 10 ? { pageSize: 10, showSizeChanger: false } : false} locale={{ emptyText: <Empty description={d('noPayments')} image={Empty.PRESENTED_IMAGE_SIMPLE} /> }} columns={[
      { title: grouping === 'cc' ? d('currentCC') : d('paidDate'), dataIndex: 'name', fixed: 'left', width: 160 },
      { title: d('paidOrders'), width: 120, render: (_, row) => link(`${row.name} · ${d('paidOrders')}`, row.orders.length, () => open('orders', row.id)) },
      { title: d('paidUsers'), width: 120, render: (_, row) => link(`${row.name} · ${d('paidUsers')}`, row.users, () => open('users', row.id)) },
      { title: d('revenue'), width: 210, render: (_, row) => link(`${row.name} · ${d('revenue')}`, money(row.amount), () => open('orders', row.id)) },
      ...
      <Table.Summary.Cell index={4}>{money(selected.averagePerUser || 0)}</Table.Summary.Cell>
    </Table.Summary.Row>} />
    {grouping === 'date' && <p className="dashboard-help dashboard-bottom-note">{d('datePeriodHelp')}</p>}
    <Modal width={1120} open={query.get('paymentDetail') === '1'} onCancel={close} title={`${d('paymentDetails')} · ${selectedGroup ? groupName(selectedGroup) : d('allPayments')}`} footer={<Button onClick={close}>{t('common.close')}</Button>}>
      <p className="dashboard-help">{d('paidDate')}：{grouping === 'date' && selectedGroup ? selectedGroup : rangeLabel} · {filters.owner ? ownerName(filters.owner) : d('allCC')} · {currency} · UTC+7<br />{d('paymentScope')}</p>
