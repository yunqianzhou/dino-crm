...
export default function DashboardConversion43({ population,calls,lessons,orders,filters,rangeLabel }: Props) {
 ...
 const exportButton = <Export43 name="conversion-cohort" disabled={!metrics.leads.length} sheets={() => [
  {name:text('用户指标','User metrics'),headers:['CRM ID',text('姓名','Name'),text('当前 CC','Current CC'),text('注册时间 UTC','Registered UTC'),...FUNNEL_KEYS.map(label)],rows:metrics.leads.map(s=>[s.studentId,s.name,ownerName(s.salesOwner||'__unassigned__'),s.registerTime,...FUNNEL_KEYS.map(k=>Number(metricIds[k].has(s.studentId)))])},
  ...
  {name:text('统计范围','Scope'),headers:['Scope','Value'],rows:[['Registration dates UTC+7',rangeLabel],['CC',Array.isArray(filters.owner)?filters.owner.join(','):filters.owner],['Business line','Vietnam'],['User type','Formal'],['Deduplication','CRM user ID; each step independent'],['L2S','Paid users / Leads'],['Exported UTC',dayjs.utc().toISOString()]]},
 ]} />
 return <>
 <Card className="dashboard-funnel-card" title={text('注册批次转化漏斗','Registration cohort funnel')} extra={exportButton}>
 ...
 <Card title={text('转化明细','Conversion details')} extra={<span className="dashboard-section-note">{text('展开查看下一层 · 金额按币种分列','Expand for the next level · currencies shown separately')}</span>}>
