...
async function putAsset(id: string, file: File) { const db=await assetDB(); try { await new Promise<void>((resolve,reject)=>{const tx=db.transaction('files','readwrite');tx.objectStore('files').put(file,id);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error)}) } finally { db.close() } }
...
export default function ABAssetField({label,value,onChange,disabled=false,allowGif=false}:{label:string;value:Asset;onChange:(v:Asset)=>void;disabled?:boolean;allowGif?:boolean}) {
