...
try {
  ...
  // Multi-CC selection is a union, including unassigned, and never broadens access.
  assert.deepEqual(dashboardOwnerIds(['cc-a', '', 'cc-a', 'cc-b']), ['cc-a', 'cc-b'])
  assert.deepEqual(dashboardMetrics(population, [], [], { ...filters, owner: ['a@example.com', 'b@example.com'] }).total.map(s => s.studentId), ['a', 'b'])
  assert.deepEqual(dashboardMetrics(population, [], [], { ...filters, owner: ['a@example.com', '__unassigned__'] }).total.map(s => s.studentId), ['a', 'pool'])
  assert.deepEqual(dashboardMetrics(population, [], [], { ...filters, owner: [] }).total.map(s => s.studentId), current.total.map(s => s.studentId))
  assert.deepEqual(dashboardMetrics(own, [], [], { ...filters, owner: ['a@example.com', 'b@example.com'] }).total.map(s => s.studentId), ['a'])
  const ccSelection = [...new Set(demo.students.map(s => s.salesOwner).filter(Boolean))].slice(0, 2)
  const selectedPeople = demo.students.filter(s => ccSelection.includes(s.salesOwner))
  for (const mode of ['current', 'period']) {
    const selectedFilters = { ...filters, mode, owner: ccSelection }
    assert.deepEqual(dashboardMetrics(demo.students, demo.callRecords, demo.lessons, selectedFilters, demo.orders), dashboardMetrics(selectedPeople, demo.callRecords, demo.lessons, { ...filters, mode }, demo.orders))
    assert.deepEqual(dashboardDateRows(demo.students, demo.callRecords, demo.lessons, selectedFilters, demo.orders), dashboardDateRows(selectedPeople, demo.callRecords, demo.lessons, { ...filters, mode }, demo.orders))
    assert.deepEqual(dashboardReasonRows(demo.students, demo.callRecords, demo.lessons, selectedFilters, 'noShow'), dashboardReasonRows(selectedPeople, demo.callRecords, demo.lessons, { ...filters, mode }, 'noShow'))
    assert.deepEqual(dashboardBreakdownPayments(demo.orders, demo.students, selectedFilters), dashboardBreakdownPayments(demo.orders, selectedPeople, { ...filters, mode }))
  }
  ...
  assert(!matchesDashboardScope(clicked, 'payer', 'excluded-0'))
  assert(!matchesDashboardScope(clicked, 'absent', 'one'))
  ...
  assert.deepEqual([...sortValues].sort((a, b) => -dashboardNumberCompare(a, b, 'descend')), [100, 12, 2, 0, null])
